package com.example.jminl.information1;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        anser_image = (ImageView) findViewById(R.id.answer_image);
        name_arr = new String[6];
        img_arr = new int[6];
        price_arr = new int[6];
        number_arr = new int[6];
        ans = new int[4];
        ran_arr = new int[5];
        ...
        img_arr[0] = R.drawable.fish;
        img_arr[1] = R.drawable.carrot;
        img_arr[2] = R.drawable.water;
        img_arr[3] = R.drawable.bread;
        img_arr[4] = R.drawable.milk;
        img_arr[5] = R.drawable.bin;
    ...
    public void play(boolean gamebool)//게임 메인 함수
    {
        for (int i = 0; i < 5; i++) {
            ...
            tv_arr[i].setText("");
            image_arr[i].setImageResource(5);
            tv_arr[i + 5].setText("");
            tv_arr[i + 10].setText("");
        }
        ...
        for (int i = 0; i < PRODUCT_NUM; i++) {
            ...
            number_arr[i] = (int) (random.nextInt(5) + 1);
        }

        for (int i = 0; i < 5; i++) {
            int ran_temp = (int) (random.nextInt(5));

            for (int j = 0; j < i; j++) {
                if (ran_arr[j] == ran_temp) {
                    ran_temp = (int) (random.nextInt(5));
                    j = 0;
                }
            }
            ran_arr[i] = ran_temp;
        ...
        switch (view.getId()) {
            case R.id.button1:
                if (ans[0] == t_ans) {//정답
                    score += 5;
                    play_num++;
                    anser_image.setVisibility(View.VISIBLE);
                    correctF(true);
                    if(play_num==20)
                        finish();
                } else {//오답
    ...
    public void correctF(boolean bool) {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                anser_image.setVisibility(View.INVISIBLE);
            }
        }, 1000);
